...
Status: operational plan; exact clock time, venue, submission deadline, judging rubric, and day-of-code rule must be confirmed at check-in
...
Public source: [Cerebral Valley event page](https://cerebralvalley.ai/e/abridge-hackathon). The full prompt and waiver were recovered from the event payload on 2026-07-16.
...
- run the `2 × 2` evidence-versus-majority experiment plus irrelevant-evidence control on a small frozen set;
- show raw `n/N`, agreement metrics, uncertainty, vote-change taxonomy, latency, and cost;
- obtain at least one independent clinician rating before they see the AI result and one structured usability review afterward;
- render a compact comparison: single model versus ordinary debate versus Tribunal.
...
### Pablo — clinical construct, partner interface, and narrative
...
### Santiago — implementation, instrumentation, and demo reliability

- create the start tag/receipt and day-of branch;
- implement the input adapter, schema validation, panel configuration, and packet UI;
- run tests, experiment assignments, metrics, and receipts;
- maintain the offline fallback and screen recording;
- make checkpoint commits and keep the remote branch current.
...
The recovered event payload encodes `2026-07-18 16:00` through `2026-07-19 05:00` with `America/Los_Angeles`; its serialization is ambiguous. A prior conversion suggested 09:00–22:00 PDT, but that must be verified from the approved registration or organizer. The relative schedule below is authoritative once `T0` is confirmed.

| Time | Goal | Exit condition |
| --- | --- | --- |
| T−45 to T0 | Check in, confirm rules/data/IP, connect power/network, make start tag and receipt | written organizer answer; clean start proof |
...
| T+8:45 to T+9:45 | Implement only the highest-value clinician fixes | second P0 run passes; changes linked to feedback |
...
| T+11:45 to T+12:30 | Final metrics, cost receipt, README, submission | clean branch; tests pass; claims audited |
| Final buffer | Record, upload, verify links, rehearse twice | submission confirmed; backup local copy; no unverified headline |
...
The minimum meaningful experiment uses the same frozen cases and prompts across conditions:

1. **Single:** one model gives the tuple.
2. **Debate:** agents see preceding recommendations before deciding.
3. **Tribunal:** agents vote independently, then revise privately under randomized evidence/social signals.

Primary Saturday endpoint: correct `escalation_action` on the planted/reference label. Secondary endpoints: false-majority flips, valid-evidence corrections, non-votes, preserved dissent, exact tuple match, latency, and model cost.

Show every numerator and denominator. If the sample is tiny, say “mechanism demonstration on N cases,” not “performance improvement.” A failed or null result remains useful if the ledger makes the mechanism diagnosable.
