...
| Artifact | Provenance | State | Disposition |
| --- | --- | --- | --- |
| Clean replacement history | Local commits `315baea` (curated squash from `main`), `1ba7ce0` (receipted run), `2b0be47` (verification ledger); backup ref `backup/pr3-old-head` preserves the old head locally | method_checked | All gates green pre-commit (tests, typecheck, build, web smoke, 13 anchors); tree privacy-scanned (accepted pre-existing absolute home-directory path strings — five occurrences in the single file `runs/demo-recording/manifest.json` — inherited byte-identical from `main`); leaked paths absent from all three trees; awaiting operator-approved force-with-lease |
