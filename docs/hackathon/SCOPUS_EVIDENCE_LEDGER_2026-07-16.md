...
| Human–AI advice timing, automation bias, anchoring | Authenticated institutional-access Scopus AI query run 2026-07-16 (Rao-prep session); raw capture local-only | grounded; selected primary records verified to the evidence level stated in §5b; paywalled effect details remain pending where labeled |
| Apples-to-apples AI-vs-physician comparison controls | Authenticated institutional-access Scopus AI query run 2026-07-16 (Rao-prep session); raw capture local-only | grounded; Chen et al. 2026 scoping-review abstract and adjacent primary records verified as scoped in §5b; AIPSC item wording remains full-text pending |
| Silent trials / shadow-mode deployments | Authenticated institutional-access Scopus AI queries run 2026-07-16 (Rao-prep session); raw capture local-only | abstract governance phrasing returned Scopus AI's own "Not based on Scopus references" banner (recorded ungrounded attempt); empirical rephrasing grounded with 7 references, with CHARTwatch and Kwong primary full text plus SAFE-WAIT design verified as scoped in §5b |
...
7. complete only the unresolved depth checks behind the now-verified §5b rows: pull Chen et al. 2026 full text before paraphrasing AIPSC item wording; recover paywalled Ns/effect sizes where §5b labels them pending; resolve the Cabitza "Judicial AI" bias-rate wording; and retrieve SAFE-WAIT subgroup numbers before quoting them;
